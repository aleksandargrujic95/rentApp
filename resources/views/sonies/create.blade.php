...
@section('content')
	...
	<form action="/sonies" method="POST" role="form" style="width: 90%; margin-bottom: 10px;">
		...
		<div class="form-group">
			...
			<input type="date" class="form-control {{ $errors->has('date_of_rent') ? 'is-invalid' :  ''}}" id="date_of_rent" name="date_of_rent" value="{{ old('date_of_rent') }}" required>
			<label for="title">Datum povratka:</label>
			<input type="date" class="form-control {{ $errors->has('date_of_giveback') ? 'is-invalid' :  ''}}" id="date_of_giveback" name="date_of_giveback" value="{{ old('date_of_giveback') }}" required>
